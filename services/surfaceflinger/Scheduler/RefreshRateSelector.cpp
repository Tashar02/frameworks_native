...
auto RefreshRateSelector::getRankedFrameRatesLocked(const std::vector<LayerRequirement>& allLayers,
    ...
    const auto selectivelyForceIdle = [&]() REQUIRES(mLock) -> RankedFrameRates {
        ...
        if (localIsIdle && !hasExplicitVoteLayers && !ranking.empty() && ranking.front().frameRateMode.fps > mIdleFps) {
            /*
        ...
        ALOGV("%s scored", ranking.empty() ? "none" : to_string(ranking.front().frameRateMode.fps).c_str());
    ...
    if (!signals.touch && signals.idle &&
        ...
        if (idleBestMode != mPrimaryFrameRates.end()) {
            ...
            ranking = {ScoredFrameRate{*idleBestMode, kMaxScore}};
        } else {
    ...
    if (layers.empty() || noVoteLayers == layers.size()) {
        ...
        if (noVoteBestMode != mPrimaryFrameRates.end()) {
            constexpr float kMaxScore = std::numeric_limits<float>::max();
            ranking = {ScoredFrameRate{*noVoteBestMode, kMaxScore}};
        } else {
